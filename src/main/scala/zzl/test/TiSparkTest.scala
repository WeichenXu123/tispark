...
object TiSparkTest {
  ...
  def runStudentTable(ti:TiContext,spark:SparkSession): Unit ={
    val df = ti.tidbTable( "test", "student")
    df.createGlobalTempView("t1")
    spark.sql("select class, avg(score) from global_temp.t1 WHERE school = 's1'  and (studentId >= 4 and studentId < 10) or studentId in (1, 2) group by class").show
  }
}
